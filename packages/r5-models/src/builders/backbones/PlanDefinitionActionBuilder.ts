...
export class PlanDefinitionActionBuilder extends BackboneElementBuilder<PlanDefinitionAction, IPlanDefinitionAction> {
  ...
  /**
   * Set timing choice type (timingAge, timingDuration, timingRange, timingTiming)
   * @param type - 'Age' | 'Duration' | 'Range' | 'Timing'
   * @param value - The value for the chosen type
   *
   * @example
   * builder.setTiming('Age', value)
   */
  setTiming<T extends 'Age' | 'Duration' | 'Range' | 'Timing'>(
    type: T,
    value: ChoiceTypeValue<T>
  ): this {
   ...
   */
  addInput(input: IPlanDefinitionActionInput): this {
   ...
   */
  addOutput(output: IPlanDefinitionActionOutput): this {
