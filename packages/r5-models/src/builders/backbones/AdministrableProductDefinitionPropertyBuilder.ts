...
export class AdministrableProductDefinitionPropertyBuilder extends BackboneElementBuilder<AdministrableProductDefinitionProperty, IAdministrableProductDefinitionProperty> {
  ...
  /**
   * Set value choice type (valueCodeableConcept, valueQuantity, valueDate, valueBoolean, valueMarkdown, valueAttachment, valueReference)
   * @param type - 'CodeableConcept' | 'Quantity' | 'Date' | 'Boolean' | 'Markdown' | 'Attachment' | 'Reference'
   * @param value - The value for the chosen type
   *
   * @example
   * builder.setValue('CodeableConcept', value)
   */
  setValue<T extends 'CodeableConcept' | 'Quantity' | 'Date' | 'Boolean' | 'Markdown' | 'Attachment' | 'Reference'>(
    type: T,
    ...
    if (type !== 'Markdown') {
      otherKeys.push('valueMarkdown' as keyof IAdministrableProductDefinitionProperty);
      otherKeys.push('_valueMarkdown' as keyof IAdministrableProductDefinitionProperty);
    }
    if (type !== 'Attachment') {
    ...
    if (type !== 'Reference') {
      otherKeys.push('valueReference' as keyof IAdministrableProductDefinitionProperty);
      otherKeys.push('_valueReference' as keyof IAdministrableProductDefinitionProperty);
    }
