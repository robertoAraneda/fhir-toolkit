...
export class TaskOutputBuilder extends BackboneElementBuilder<TaskOutput, ITaskOutput> {
  ...
  setValue<T extends 'Base64Binary' | 'Boolean' | 'Canonical' | 'Code' | 'Date' | 'DateTime' | 'Decimal' | 'Id' | 'Instant' | 'Integer' | 'Integer64' | 'Markdown' | 'Oid' | 'PositiveInt' | 'String' | 'Time' | 'UnsignedInt' | 'Uri' | 'Url' | 'Uuid' | 'Address' | 'Age' | 'Annotation' | 'Attachment' | 'CodeableConcept' | 'CodeableReference' | 'Coding' | 'ContactPoint' | 'Count' | 'Distance' | 'Duration' | 'HumanName' | 'Identifier' | 'Money' | 'Period' | 'Quantity' | 'Range' | 'Ratio' | 'RatioRange' | 'Reference' | 'SampledData' | 'Signature' | 'Timing' | 'ContactDetail' | 'DataRequirement' | 'Expression' | 'ParameterDefinition' | 'RelatedArtifact' | 'TriggerDefinition' | 'UsageContext' | 'Availability' | 'ExtendedContactDetail' | 'Dosage' | 'Meta'>(
    ...
    if (type !== 'Integer64') {
      otherKeys.push('valueInteger64' as keyof ITaskOutput);
      otherKeys.push('_valueInteger64' as keyof ITaskOutput);
    }
    if (type !== 'Markdown') {
    ...
    if (type !== 'CodeableReference') {
      otherKeys.push('valueCodeableReference' as keyof ITaskOutput);
      otherKeys.push('_valueCodeableReference' as keyof ITaskOutput);
    }
    if (type !== 'Coding') {
    ...
    if (type !== 'RatioRange') {
      otherKeys.push('valueRatioRange' as keyof ITaskOutput);
      otherKeys.push('_valueRatioRange' as keyof ITaskOutput);
    }
    if (type !== 'Reference') {
    ...
    if (type !== 'Availability') {
      otherKeys.push('valueAvailability' as keyof ITaskOutput);
      otherKeys.push('_valueAvailability' as keyof ITaskOutput);
    }
    if (type !== 'ExtendedContactDetail') {
      otherKeys.push('valueExtendedContactDetail' as keyof ITaskOutput);
      otherKeys.push('_valueExtendedContactDetail' as keyof ITaskOutput);
    }
    if (type !== 'Dosage') {
