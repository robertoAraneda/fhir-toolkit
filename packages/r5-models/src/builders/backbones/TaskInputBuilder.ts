...
export class TaskInputBuilder extends BackboneElementBuilder<TaskInput, ITaskInput> {
  ...
  setValue<T extends 'Base64Binary' | 'Boolean' | 'Canonical' | 'Code' | 'Date' | 'DateTime' | 'Decimal' | 'Id' | 'Instant' | 'Integer' | 'Integer64' | 'Markdown' | 'Oid' | 'PositiveInt' | 'String' | 'Time' | 'UnsignedInt' | 'Uri' | 'Url' | 'Uuid' | 'Address' | 'Age' | 'Annotation' | 'Attachment' | 'CodeableConcept' | 'CodeableReference' | 'Coding' | 'ContactPoint' | 'Count' | 'Distance' | 'Duration' | 'HumanName' | 'Identifier' | 'Money' | 'Period' | 'Quantity' | 'Range' | 'Ratio' | 'RatioRange' | 'Reference' | 'SampledData' | 'Signature' | 'Timing' | 'ContactDetail' | 'DataRequirement' | 'Expression' | 'ParameterDefinition' | 'RelatedArtifact' | 'TriggerDefinition' | 'UsageContext' | 'Availability' | 'ExtendedContactDetail' | 'Dosage' | 'Meta'>(
    ...
    if (type !== 'Integer64') {
      otherKeys.push('valueInteger64' as keyof ITaskInput);
      otherKeys.push('_valueInteger64' as keyof ITaskInput);
    }
    if (type !== 'Markdown') {
    ...
    if (type !== 'CodeableReference') {
      otherKeys.push('valueCodeableReference' as keyof ITaskInput);
      otherKeys.push('_valueCodeableReference' as keyof ITaskInput);
    }
    if (type !== 'Coding') {
    ...
    if (type !== 'RatioRange') {
      otherKeys.push('valueRatioRange' as keyof ITaskInput);
      otherKeys.push('_valueRatioRange' as keyof ITaskInput);
    }
    if (type !== 'Reference') {
    ...
    if (type !== 'Availability') {
      otherKeys.push('valueAvailability' as keyof ITaskInput);
      otherKeys.push('_valueAvailability' as keyof ITaskInput);
    }
    if (type !== 'ExtendedContactDetail') {
      otherKeys.push('valueExtendedContactDetail' as keyof ITaskInput);
      otherKeys.push('_valueExtendedContactDetail' as keyof ITaskInput);
    }
    if (type !== 'Dosage') {
