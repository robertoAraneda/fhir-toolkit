...
export class QuestionnaireBuilder extends DomainResourceBuilder<Questionnaire, IQuestionnaire> {
  ...
  setVersionAlgorithm<T extends 'String' | 'Coding'>(
    ...
    const key = `versionAlgorithm${type}` as keyof IQuestionnaire;
    const otherKeys: (keyof IQuestionnaire)[] = [];
    if (type !== 'String') {
      otherKeys.push('versionAlgorithmString' as keyof IQuestionnaire);
      otherKeys.push('_versionAlgorithmString' as keyof IQuestionnaire);
    }
    if (type !== 'Coding') {
      otherKeys.push('versionAlgorithmCoding' as keyof IQuestionnaire);
      otherKeys.push('_versionAlgorithmCoding' as keyof IQuestionnaire);
    }
