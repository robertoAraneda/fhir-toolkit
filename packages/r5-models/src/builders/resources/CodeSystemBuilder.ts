...
export class CodeSystemBuilder extends DomainResourceBuilder<CodeSystem, ICodeSystem> {
  ...
  setVersionAlgorithm<T extends 'String' | 'Coding'>(
    ...
    const key = `versionAlgorithm${type}` as keyof ICodeSystem;
    const otherKeys: (keyof ICodeSystem)[] = [];
    if (type !== 'String') {
      otherKeys.push('versionAlgorithmString' as keyof ICodeSystem);
      otherKeys.push('_versionAlgorithmString' as keyof ICodeSystem);
    }
    if (type !== 'Coding') {
      otherKeys.push('versionAlgorithmCoding' as keyof ICodeSystem);
      otherKeys.push('_versionAlgorithmCoding' as keyof ICodeSystem);
    }
