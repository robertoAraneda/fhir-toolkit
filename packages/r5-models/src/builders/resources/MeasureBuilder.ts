...
export class MeasureBuilder extends DomainResourceBuilder<Measure, IMeasure> {
  ...
  setVersionAlgorithm<T extends 'String' | 'Coding'>(
    ...
    const key = `versionAlgorithm${type}` as keyof IMeasure;
    const otherKeys: (keyof IMeasure)[] = [];
    if (type !== 'String') {
      otherKeys.push('versionAlgorithmString' as keyof IMeasure);
      otherKeys.push('_versionAlgorithmString' as keyof IMeasure);
    }
    if (type !== 'Coding') {
      otherKeys.push('versionAlgorithmCoding' as keyof IMeasure);
      otherKeys.push('_versionAlgorithmCoding' as keyof IMeasure);
    }
   ...
   */
  addTerm(term: IMeasureTerm): this {
