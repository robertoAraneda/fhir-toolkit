...
export class StructureDefinitionBuilder extends DomainResourceBuilder<StructureDefinition, IStructureDefinition> {
  ...
  setVersionAlgorithm<T extends 'String' | 'Coding'>(
    ...
    const key = `versionAlgorithm${type}` as keyof IStructureDefinition;
    const otherKeys: (keyof IStructureDefinition)[] = [];
    if (type !== 'String') {
      otherKeys.push('versionAlgorithmString' as keyof IStructureDefinition);
      otherKeys.push('_versionAlgorithmString' as keyof IStructureDefinition);
    }
    if (type !== 'Coding') {
      otherKeys.push('versionAlgorithmCoding' as keyof IStructureDefinition);
      otherKeys.push('_versionAlgorithmCoding' as keyof IStructureDefinition);
    }
