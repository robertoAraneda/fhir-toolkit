...
export class PlanDefinitionBuilder extends DomainResourceBuilder<PlanDefinition, IPlanDefinition> {
  ...
  setVersionAlgorithm<T extends 'String' | 'Coding'>(
    ...
    const key = `versionAlgorithm${type}` as keyof IPlanDefinition;
    const otherKeys: (keyof IPlanDefinition)[] = [];
    if (type !== 'String') {
      otherKeys.push('versionAlgorithmString' as keyof IPlanDefinition);
      otherKeys.push('_versionAlgorithmString' as keyof IPlanDefinition);
    }
    if (type !== 'Coding') {
      otherKeys.push('versionAlgorithmCoding' as keyof IPlanDefinition);
      otherKeys.push('_versionAlgorithmCoding' as keyof IPlanDefinition);
    }
  ...
  setAsNeeded<T extends 'Boolean' | 'CodeableConcept'>(
    ...
    const key = `asNeeded${type}` as keyof IPlanDefinition;
    const otherKeys: (keyof IPlanDefinition)[] = [];
    if (type !== 'Boolean') {
      otherKeys.push('asNeededBoolean' as keyof IPlanDefinition);
      otherKeys.push('_asNeededBoolean' as keyof IPlanDefinition);
    }
    if (type !== 'CodeableConcept') {
      otherKeys.push('asNeededCodeableConcept' as keyof IPlanDefinition);
      otherKeys.push('_asNeededCodeableConcept' as keyof IPlanDefinition);
    }
    return this.setChoiceType(key, value, otherKeys);
  }
   ...
   */
  addActor(actor: IPlanDefinitionActor): this {
