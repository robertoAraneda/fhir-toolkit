...
export class SubscriptionTopicBuilder extends DomainResourceBuilder<SubscriptionTopic, ISubscriptionTopic> {
  ...
  setVersionAlgorithm<T extends 'String' | 'Coding'>(
    ...
    const key = `versionAlgorithm${type}` as keyof ISubscriptionTopic;
    const otherKeys: (keyof ISubscriptionTopic)[] = [];
    if (type !== 'String') {
      otherKeys.push('versionAlgorithmString' as keyof ISubscriptionTopic);
      otherKeys.push('_versionAlgorithmString' as keyof ISubscriptionTopic);
    }
    if (type !== 'Coding') {
      otherKeys.push('versionAlgorithmCoding' as keyof ISubscriptionTopic);
      otherKeys.push('_versionAlgorithmCoding' as keyof ISubscriptionTopic);
    }
