...
export class ActivityDefinitionBuilder extends DomainResourceBuilder<ActivityDefinition, IActivityDefinition> {
   ...
   * Kind of resource
   */
  setKind(kind: RequestResourceTypesType): this {
    this.data.kind = kind;
    return this;
  }
  ...
  setVersionAlgorithm<T extends 'String' | 'Coding'>(
    ...
    const key = `versionAlgorithm${type}` as keyof IActivityDefinition;
    const otherKeys: (keyof IActivityDefinition)[] = [];
    if (type !== 'String') {
      otherKeys.push('versionAlgorithmString' as keyof IActivityDefinition);
      otherKeys.push('_versionAlgorithmString' as keyof IActivityDefinition);
    }
    if (type !== 'Coding') {
      otherKeys.push('versionAlgorithmCoding' as keyof IActivityDefinition);
      otherKeys.push('_versionAlgorithmCoding' as keyof IActivityDefinition);
    }
  ...
  /**
   * Set timing choice type (timingTiming, timingAge, timingRange, timingDuration)
   * @param type - 'Timing' | 'Age' | 'Range' | 'Duration'
   * @param value - The value for the chosen type
   *
   * @example
   * builder.setTiming('Timing', value)
   */
  setTiming<T extends 'Timing' | 'Age' | 'Range' | 'Duration'>(
    type: T,
    value: ChoiceTypeValue<T>
  ): this {
  ...
  setAsNeeded<T extends 'Boolean' | 'CodeableConcept'>(
    ...
    const key = `asNeeded${type}` as keyof IActivityDefinition;
    const otherKeys: (keyof IActivityDefinition)[] = [];
    if (type !== 'Boolean') {
      otherKeys.push('asNeededBoolean' as keyof IActivityDefinition);
      otherKeys.push('_asNeededBoolean' as keyof IActivityDefinition);
    }
    if (type !== 'CodeableConcept') {
      otherKeys.push('asNeededCodeableConcept' as keyof IActivityDefinition);
      otherKeys.push('_asNeededCodeableConcept' as keyof IActivityDefinition);
    }
    return this.setChoiceType(key, value, otherKeys);
  }
   ...
   */
  addSpecimenRequirement(specimenRequirement: string): this {
   ...
   */
  addObservationRequirement(observationRequirement: string): this {
   ...
   */
  addObservationResultRequirement(observationResultRequirement: string): this {
