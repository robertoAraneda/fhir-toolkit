...
export class LibraryBuilder extends DomainResourceBuilder<Library, ILibrary> {
  ...
  setVersionAlgorithm<T extends 'String' | 'Coding'>(
    ...
    const key = `versionAlgorithm${type}` as keyof ILibrary;
    const otherKeys: (keyof ILibrary)[] = [];
    if (type !== 'String') {
      otherKeys.push('versionAlgorithmString' as keyof ILibrary);
      otherKeys.push('_versionAlgorithmString' as keyof ILibrary);
    }
    if (type !== 'Coding') {
      otherKeys.push('versionAlgorithmCoding' as keyof ILibrary);
      otherKeys.push('_versionAlgorithmCoding' as keyof ILibrary);
    }
