...
export class ExampleScenarioBuilder extends DomainResourceBuilder<ExampleScenario, IExampleScenario> {
  ...
  setVersionAlgorithm<T extends 'String' | 'Coding'>(
    ...
    const key = `versionAlgorithm${type}` as keyof IExampleScenario;
    const otherKeys: (keyof IExampleScenario)[] = [];
    if (type !== 'String') {
      otherKeys.push('versionAlgorithmString' as keyof IExampleScenario);
      otherKeys.push('_versionAlgorithmString' as keyof IExampleScenario);
    }
    if (type !== 'Coding') {
      otherKeys.push('versionAlgorithmCoding' as keyof IExampleScenario);
      otherKeys.push('_versionAlgorithmCoding' as keyof IExampleScenario);
    }
