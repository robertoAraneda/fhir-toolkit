...
/**
 * EventDefinitionBuilder - Fluent builder for EventDefinition resources
 *
 ...
 *
 * @example
 * const eventDefinition = new EventDefinitionBuilder()
 *   .setId('123')
 *   .setUrl(value)
 *   .addIdentifier({ ... })
 *   .build();
 */
export class EventDefinitionBuilder extends DomainResourceBuilder<EventDefinition, IEventDefinition> {
  // ============================================================================
  // Scalar Properties
  // ============================================================================
  ...
  setVersionAlgorithm<T extends 'String' | 'Coding'>(
    ...
    const key = `versionAlgorithm${type}` as keyof IEventDefinition;
    const otherKeys: (keyof IEventDefinition)[] = [];
    if (type !== 'String') {
      otherKeys.push('versionAlgorithmString' as keyof IEventDefinition);
      otherKeys.push('_versionAlgorithmString' as keyof IEventDefinition);
    }
    if (type !== 'Coding') {
      otherKeys.push('versionAlgorithmCoding' as keyof IEventDefinition);
      otherKeys.push('_versionAlgorithmCoding' as keyof IEventDefinition);
    }
  ...
  setSubject<T extends 'CodeableConcept' | 'Reference'>(
    ...
    if (type !== 'CodeableConcept') {
      otherKeys.push('subjectCodeableConcept' as keyof IEventDefinition);
      otherKeys.push('_subjectCodeableConcept' as keyof IEventDefinition);
    }
    if (type !== 'Reference') {
      otherKeys.push('subjectReference' as keyof IEventDefinition);
      otherKeys.push('_subjectReference' as keyof IEventDefinition);
    }
    return this.setChoiceType(key, value, otherKeys);
  }
  ...
  /**
   * Build the EventDefinition instance
   */
  build(): EventDefinition {
    return new EventDefinition(this.data);
  }

  /**
   * Build and validate the EventDefinition instance
   * @throws Error if validation fails
   */
  async buildOrThrow(): Promise<EventDefinition> {
    const eventDefinition = this.build();
    await eventDefinition.validateOrThrow();
