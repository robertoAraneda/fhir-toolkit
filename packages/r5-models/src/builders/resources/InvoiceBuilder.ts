...
export class InvoiceBuilder extends DomainResourceBuilder<Invoice, IInvoice> {
  ...
  /**
   * Set period choice type (periodDate, periodPeriod)
   * @param type - 'Date' | 'Period'
   * @param value - The value for the chosen type
   *
   * @example
   * builder.setPeriod('Date', value)
   */
  setPeriod<T extends 'Date' | 'Period'>(
    type: T,
    value: ChoiceTypeValue<T>
  ): this {
    const key = `period${type}` as keyof IInvoice;
    const otherKeys: (keyof IInvoice)[] = [];
    if (type !== 'Date') {
      otherKeys.push('periodDate' as keyof IInvoice);
      otherKeys.push('_periodDate' as keyof IInvoice);
    }
    if (type !== 'Period') {
      otherKeys.push('periodPeriod' as keyof IInvoice);
      otherKeys.push('_periodPeriod' as keyof IInvoice);
    }
   ...
   */
  addTotalPriceComponent(totalPriceComponent: IMonetaryComponent): this {
