...
export class MessageHeaderBuilder extends DomainResourceBuilder<MessageHeader, IMessageHeader> {
   ...
   */
  setSender(sender: IReference<'Practitioner' | 'PractitionerRole' | 'Organization'>): this {
  ...
  setEvent<T extends 'Coding' | 'Uri'>(
    ...
    if (type !== 'Uri') {
      otherKeys.push('eventUri' as keyof IMessageHeader);
      otherKeys.push('_eventUri' as keyof IMessageHeader);
    }
