/**
 * Test utilities for FHIR Toolkit
 *
...
/**
 * Helper to create a test suite for common model behaviors.
 */
export function describeModelBehaviors<
  TModel extends {
    toJSON(): TJson;
    clone(): TModel;
    with(changes: Partial<TJson>): TModel;
    toString(): string;
  },
  TJson extends object
>(
  ModelClass: {
    new (data?: Partial<TJson>): TModel;
    fromJSON(json: TJson): TModel;
  },
  sampleData: Partial<TJson>,
  modelName: string
): void {
  describe(`${modelName} Common Behaviors`, () => {
    it('should create instance from constructor', () => {
      const instance = new ModelClass(sampleData);
      expect(instance).toBeInstanceOf(ModelClass);
    });
    ...
    it('should create instance from JSON', () => {
      ...
      expect(restored).toBeInstanceOf(ModelClass);
      expect(restored.toJSON()).toEqual(json);
    });

    it('should clone correctly', () => {
      const original = new ModelClass(sampleData);
    ...
    it('should return string from toString', () => {
      const instance = new ModelClass(sampleData);
      expect(typeof instance.toString()).toBe('string');
    });
  });
}
