/**
 * Basic Validation Tests for FHIR R4
 *
 * Tests basic validation functionality against FHIR R4 specs.
 */

import { describe, it, expect, beforeAll } from 'vitest';
import { FhirValidator } from '../../src/index.js';

describe('FHIR R4 Basic Validation', () => {
  let validator: FhirValidator;

  beforeAll(async () => {
    // Explicitly create an R4 validator
    validator = new FhirValidator({ fhirVersion: 'R4' });
    await validator.initialize();
  });

  it('should be configured for R4', () => {
    expect(validator.getFhirVersion()).toBe('R4');
  });
